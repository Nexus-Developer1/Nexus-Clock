...
- Rota `/projetos/{id}` (`projetos.ver`), componente `App\Livewire\Projetos\Detalhe`. Abre-se pelo **nome** na lista de Projetos (não pela linha, que tem a estrela dos favoritos e as ações) e pelos nomes dos projetos na página do cliente (§35 deixava isso para quando houvesse página de projeto).

...
- O deploy passa a ser, com `A="sudo -u app-tempos env HOME=/var/lib/nexus-apps/app-tempos"`:
  `cd /var/www/nexus-tempos`, `$A git pull --ff-only origin feature/tempos`, `$A php artisan migrate --force`, `optimize:clear`, `config:cache`, `event:cache`, `view:cache`, `queue:restart`.
...
- **setgid nas pastas, obrigatório:** a pasta é 750 e o Apache lê o `public/` pelo grupo `www-data`, a que o `app-tempos` não pertence. Os ficheiros que o git criar nascem com o grupo do `app-tempos` e o CSS passa a 403, sem recuperação possível por `chgrp` (um não-root só pode dar ficheiros a grupos a que pertence). `sudo find /var/www/nexus-tempos -type d -exec chmod g+s {} +` — **tem de ser root**: um `chmod g+s` feito por quem não pertence ao grupo é descartado pelo kernel em silêncio, sem erro. Na Nexus Ops o problema não existe porque lá a pasta é 755.

...
- Rota `/clientes/{id}` (`clientes.ver`, só números — para não apanhar o `/clientes/novo`), componente `App\Livewire\Clientes\Detalhe`. Na listagem, primeiro era só **o nome** o link (a linha tem a caixa de seleção, o lápis e o «Mais opções»); a 2026-09-25, a pedido e como nos Projetos (§47), passou a ser **a linha toda** — esses três param o clique (`@click.stop`) e o nome continua a ser um link para o teclado e para «abrir num separador».
...
## 47. Página de cada projeto (2026-09-25, a pedido)

- Pedido: os projetos da lista têm de ser clicáveis e mostrar a informação de cada um, «equipa, etc.» — o conteúdo ficou à minha escolha.
- Rota `/projetos/{id}` (`projetos.ver`), componente `App\Livewire\Projetos\Detalhe`. Abre-se pela **linha toda** na lista de Projetos (primeiro era só pelo nome; a pedido, a linha inteira — a caixa de seleção, a estrela e o menu ⋯ param o clique, e o nome continua a ser um link para o teclado e para «abrir num separador») e pelos nomes dos projetos na página do cliente (§35 deixava isso para quando houvesse página de projeto).
- Mostra: cabeçalho com cor, nome, cliente (link para a página dele) e as etiquetas Arquivado / Privado / Não faturável; quatro indicadores — horas registadas (e quantas pessoas), horas faturáveis com a percentagem, progresso face à estimativa (a vermelho acima de 100%), despesas aprovadas e pendentes; os dados (acesso e, se privado, os membros; faturável; estimativa; nota; data de criação); a **Equipa** — quem registou horas no projeto, quantas, quantas faturáveis e o último registo; e as **atribuições** em curso e futuras (as que já acabaram ficam de fora). Botões para os registos do projeto no Detalhado (do ano) e para o relatório Atribuições filtrado.
- Quem vê: o mesmo `visiveisPara` da lista — um projeto **privado dá 404** a quem não é membro, sem revelar que existe; arquivado abre, apagado dá 404. As horas por pessoa já eram visíveis a toda a gente nas Atribuições (§43). **Taxa e valor em euros só para quem gere os projetos**, como na lista.
- Contam só registos terminados (um cronómetro a correr não tem duração); horas faturáveis só se o projeto o for (a mesma regra dos relatórios).
- Não há «Alterar» na página: o formulário do projeto (membros, taxa, cor…) vive na lista. Se for preciso, é trazê-lo para aqui como se fez nos clientes.

## 48. Página de cada membro da equipa (2026-09-25, a pedido)

- Pedido: em Equipa › Membros, a linha também tem de ser clicável, como nos Clientes e nos Projetos. Não havia página de membro: fez-se.
- Rota `/equipa/{id}` (`equipa.ver`, só números — não apanha `/equipa/limitados`, `/grupos`, `/lembretes`), componente `App\Livewire\Equipa\Detalhe`. Mostra: nome, email, papel; indicadores — horas desta semana face à capacidade semanal (capacidade diária × dias de trabalho), horas do mês, faturáveis do mês com a percentagem, atribuições em curso e futuras; os dados (papel, grupos, gestor de equipa, dias de trabalho, capacidade diária, início da semana; **taxas só para quem gere a equipa**, como na lista); os **projetos do mês** com as horas de cada um (link para a página do projeto, §47); e as atribuições em curso e futuras. Um membro limitado abre com uma nota (não regista horas).
- **Privacidade:** toda a gente vê a Equipa (§18) e as horas por pessoa e por projeto já são visíveis a todos nas Atribuições (§43). Mas um **projeto privado de que quem vê não é membro não aparece pelo nome**: as horas contam no total e vão para a linha «Outros projetos (privados)», sem link; nas atribuições aparece «Projeto privado». Sem isto, a página de um colega revelava projetos que a pessoa não pode ver. «Ver registos» (o Detalhado filtrado) só aparece a quem vê as horas dos outros, ou na própria página.
- **A linha clicável:** a lista tem muitos controlos para quem gere (caixas, papel, grupos, dias, menus). Em vez de um `@click.stop` em cada um, o clique na linha só navega se não tiver sido num link, botão, campo, `label` ou menu (`$event.target.closest(...)`) — os controlos que vierem ficam protegidos na mesma. Conferido que não há nenhum `div`/`span` clicável na linha. O nome é também um link.

## 49. Painel: os cartões do resumo abrem a página correspondente (2026-09-25, a pedido)

- Pedido: os quatro cartões de cima do Painel passam a ser clicáveis e a levar à página correspondente.
- **Tempo total** abre o Detalhado com o mesmo período e o mesmo «quem» do Painel (a ligação que as barras já usavam). **Faturável** abre esse Detalhado com o filtro de estado «Faturável». **Projeto principal** abre a página do projeto (§47); **Cliente principal** abre a do cliente (§35). Ao passar o rato, a borda fica verde e aparece uma seta; o cartão inteiro é um link (vai por teclado e abre num separador).
- O `PainelTempos` passa a devolver também o `id` do projeto e do cliente principais. Só se liga ao que quem vê pode abrir: se o projeto principal é privado e a pessoa já não é membro, ou se o projeto ou o cliente foi arquivado, o cartão mostra o nome como antes mas fica sem ligação — a página daria 404.
- **O cartão do projeto não abria (logo a seguir):** o projeto principal do utilizador era «Instalação data center», **privado**, com três membros — e ele não era um deles, mas tinha lá 38 h. Não era o cartão: eram os **dados de demonstração** (§39), que escolhiam o projeto de cada registo entre todos os ativos, privados incluídos, sem ver se a pessoa era membro. Na aplicação isso não acontece (só se regista no que se vê). O `tempos:demo` passa a escolher, para cada pessoa, só os projetos públicos e os privados de que é membro — nos registos, nas despesas e nas atribuições (teste com mais pessoas do que membros).
- Em produção, em vez de apagar e recriar a demonstração (perdia-se o que o utilizador foi mexendo nela), corrigiram-se só as linhas erradas, e só entre as listadas em `dados-demo.json`: 254 registos passaram para um projeto público de demonstração (com uma descrição desse projeto) e 1 despesa para o projeto público seguinte; nenhuma atribuição estava errada. Verificado depois: ninguém tem horas, despesas ou atribuições num projeto privado de que não é membro.

## 50. Detalhado: projeto, cliente e a linha clicáveis (2026-09-25, a pedido)

- Pedido: «deixa clicável aqui também», no relatório Detalhado, a seguir às páginas de projeto (§47), cliente (§35) e membro (§48) e aos cartões do Painel (§49).
- Em cada linha, o **nome do projeto** abre a página do projeto e o **nome do cliente** abre a do cliente. Só há ligação para o que quem vê pode abrir: um projeto privado de que já não é membro (pode ter horas lá de antes) e um projeto ou cliente apagado ficam em texto, como antes.
...
## 52. Segunda revisão de segurança, lote A (2026-09-25)

- Revisão pedida pelo utilizador («demora o tempo que precisares»): quatro análises em paralelo (registos e relatórios; equipa, projetos e clientes; despesas, partilhados e emails; base, configuração e dependências) mais o servidor, só a ler. Cruzada com uma outra revisão feita no mesmo dia, que deixou provas em `docs/seguranca/2026-09-25/` (não é nossa; não se mexeu nela). Nada grave — ninguém mexe no que não pode, aprova sem ser o Paulo, nem entra sem acesso —, mas vários sítios onde um técnico **via** o que não devia, e abusos possíveis. `composer audit` e `npm audit`: nada. O lote A é o que não precisa de decisões; o lote B (emails agendados, o que se muda num link partilhado, recibos antigos) fica para o utilizador decidir.
- **Projetos privados nas Atribuições.** O relatório (aberto a todos, §43) juntava atribuições e horas sem passar pelo `visiveisPara`: um técnico via o nome dos projetos privados de que não é membro, as horas dos colegas neles e as notas das atribuições. E o formulário abria-se mudando o `editarId` pelo browser (a correção da §42 pôs o gate no `nova()`/`editar()`, mas não no campo) — com a lista de todos os projetos. Agora o `RelatorioAtribuicoes` recebe quem vê: os que não pode ver juntam-se em «Outros projetos (privados)» (também no agrupar por cliente), as atribuições deles ficam «Projeto privado» sem nota, e um id desses posto à mão no filtro não filtra. `editarId` fica `#[Locked]` e o formulário só existe para quem gere. O mesmo nas **Despesas**: «Projeto privado», sem nome nem cliente, na lista, no detalhe e no CSV (`ProjetoTempo::mascarado()`, um projeto sem nome que não se grava).
- **Aprovar o que se viu.** O Paulo via 25 € na lista; entretanto o técnico mudava para 250 € (pendente, sem aviso); o «Aprovar» aprovava os 250 €. O botão leva agora a `versao()` da despesa que estava no ecrã (impressão digital de valor, data, recibo, projeto, categoria, nota e estado) e o `decidir` recusa se mudou: «Esta despesa foi alterada entretanto: veja-a de novo antes de aprovar.» E a corrida entre pedidos: `atualizar`, `apagar` e `decidir` bloqueiam a linha (`lockForUpdate`) e autorizam sobre o estado atual, não sobre a cópia que se leu antes — uma alteração com uma leitura antiga já não passa por cima de uma aprovação.
- **Pequenas fugas:** o filtro de etiquetas mostrava as de toda a gente (agora só as das horas que a pessoa vê); ordenar por valor, escondido, deixava adivinhar as taxas (Detalhado, Resumo, Projetos — só ordena por valor quem o vê); o filtro «com/sem taxa» da Equipa dizia quem tem taxa (só para quem gere); a edição em massa com ids de outros dizia o dia e o cliente de um registo alheio na mensagem de erro (agora uma mensagem genérica, sem nada).
- **Páginas abertas:** a página do projeto só verificava o acesso ao abrir, e o Painel só ao abrir e ao mudar filtros — quem saía de um projeto privado, ou deixava de ser admin, com a página aberta, continuava a ver o que entrava depois. Verificam em cada pedido. E uma segunda linha por trás do middleware persistente (§42): um gancho do Livewire (`hydrate`) dá 403 a qualquer ação sem acesso à aplicação — o middleware só se aplica quando o caminho de origem da página é uma rota desta aplicação. O relatório partilhado é a exceção (§40).
- **Exportações:** 20 por minuto por pessoa (ou por endereço, num link partilhado), dentro do `Csv::resposta()` e do `Pdf::resposta()`, por onde passam todas. O **ZIP dos recibos** ia pela ação do Livewire — inteiro em base64 dentro da resposta, com todos os recibos em memória: um ano de fotografias esgotava os 512 MB. Monta-se agora no disco (recibos pelo caminho, não pelo conteúdo), com teto (`TEMPOS_ZIP_RECIBOS_MAX_MB`, 500), e o browser vai buscá-lo a um link assinado de 10 minutos (`ZipRecibosController`), só de quem o pediu, apagado depois; os esquecidos saem ao fim de uma hora. 3 ZIPs por minuto.
- **Email ao aprovador:** o link «Abrir no Nexus Suporte» fazia-se com o endereço do pedido (cabeçalho Host). O Apache já recusa hosts estranhos (vhost `000-bloqueado`, verificado) e a aplicação não confia no `X-Forwarded-Host`, mas o link passa a fazer-se do `APP_URL`. Não se fixou o endereço da aplicação toda (`forceRootUrl`): mexia em todos os endereços, incluindo os do Livewire, para proteger o que o Apache já protege.
- **Servidor e operação:**
  - Cabeçalhos `X-Frame-Options: SAMEORIGIN`, `X-Content-Type-Options: nosniff` e `Referrer-Policy: same-origin` (os mesmos que o vhost dá ao portal e aos procedimentos; o bloco /tempos não os tinha), postos pela aplicação (`CabecalhosSeguranca`), sem mexer no vhost partilhado.
  - **Deploy apagava a cache da Nexus Infra:** o `optimize:clear` corre o `cache:clear`, que faz `FLUSHDB` à base Redis 1 — a da cache da Nexus Infra também (estado da sincronização com o ERP, `erp-sync:*`). As sessões estão na base 0 (com o `SESSION_CONNECTION` vazio o Laravel manda-as para a ligação `default`) e não eram afetadas. O deploy e o `instalar.sh` passam a limpar cada coisa à parte (`config:clear`, `event:clear`, `view:clear`, `route:clear`). A Nexus Infra faz o mesmo nos deploys dela — avisar a sessão de lá.
  - **Permissões:** 85 ficheiros de código (e a pasta `resources/views/pdf`) tinham escrita para o grupo `www-data` — o do Apache: comprometido o Apache por outra via, passava-se a correr código como o Suporte. Vinha do `umask` 002 dos `git pull` com o setgid das pastas. Tirada a escrita ao grupo (sem root: o dono é o `app-tempos`), o `.env` deixou de ser legível pelo `www-data` (lote 4 da primeira revisão) e os deploys correm com `umask 022`. O `instalar.sh` deixa `www-data` só a ler o `public/`, corre o `composer` e o `artisan` como `app-tempos` (antes o `composer` corria como root) e copia a `APP_KEY` da Nexus Ops em vez do `key:generate` (que partia a sessão partilhada numa instalação de raiz).
  - O `deploy/portal.sql` dava os acessos a partir da Nexus Infra sempre que corria — repetir o instalador devolvia o acesso (e o papel de admin) a quem o portal o tinha tirado. Só os dá agora quando a aplicação é registada nessa mesma execução.
  - `tempos:demo` numa base que não seja `tempos_dev`/`tempos_testing` pede `--em-producao` (as horas falsas entram nos lembretes, nos relatórios enviados por email e nos valores).
  - Configuração: o disco privado deixa de se servir por URL (`serve` a `false`); uploads temporários do Livewire até 10 MB e 10 por minuto (eram 12 MB e 60); `.env.*` no `.gitignore`.
- Testes: `SegurancaRevisaoTest`, um por correção, corridos **sem as correções** (falharam os 17) e com elas. Das 7 provas da outra revisão, 6 deixaram de reproduzir; a 7.ª (a limpeza da cache) verifica a configuração e continua verdadeira — a correção é no procedimento de deploy.
- Verificado e sem problema: SQL (tudo por bindings ou listas fechadas), XSS (um só `{!! !!}`, com `e()`; cores de lista fechada), recibos (caminho do servidor, disco privado, sempre como anexo), PDF (`enable_remote` e `enable_php` desligados), tokens dos links (238 bits), CSRF sem exceções, `APP_DEBUG=false`, TLS só 1.2/1.3, `.env`/`.git`/logs não acessíveis por fora, portas de base e Redis só em localhost.

## 53. Segunda revisão de segurança, lote B: as decisões do utilizador (2026-09-25)

- As quatro decisões do lote B (§52), explicadas ao utilizador com uma recomendação cada:
  1. **Emails agendados dos relatórios** (qualquer técnico agenda envios para até 10 endereços quaisquer, pela caixa da empresa, com o nome do relatório no assunto): **não se muda**, a pedido. Fica como estava — incluindo os destinatários irem todos no mesmo email. O risco (phishing com o nome da empresa, bloqueio da caixa pela Microsoft) está dito em §52; para fechar: só admins, ou técnicos só para @nxs.pt, com limite por pessoa.
  2. **Link partilhado:** quem abre vê **só o agrupamento que o autor escolheu** (e a estimativa como ele a deixou). Antes podia trocar para «membro» ou «descrição» — as horas de cada técnico e o texto interno dos registos. O `Partilhado::normalizar()` impõe os três do que o autor guardou, em cada pedido; na página os seletores dão lugar a «Agrupado por …». O período continua a mudar, salvo se o autor o bloqueou.
  3. **Recibos antigos:** trocar ou retirar o recibo de uma despesa **pendente ou rejeitada** apaga o ficheiro antigo do disco, depois de gravada a alteração. As aprovadas estão fechadas (não chegam lá); apagar uma despesa **guarda** o recibo (pode voltar a ser precisa). Os ficheiros que já tinham ficado órfãos não se apagaram.
  4. **O Paulo aprovar as próprias despesas:** «só o Paulo é que aprova, não precisa de mais ninguém» — fica como está. De caminho, o email ao aprovador diz agora quem agiu quando não foi a própria pessoa: «Julio Santos lançou, em nome de Ana Martins, uma despesa…» (ou «corrigiu, em nome de…»).

## 54. Cache do Suporte numa base Redis só dele (2026-09-25)

- Seguimento da §52: os deploys do Suporte (`optimize:clear` → `cache:clear` → `FLUSHDB`) apagavam a base Redis 1, que é a cache da Nexus Infra. Mensagem à sessão da Nexus Infra; resposta: os deploys de lá usam só `php artisan optimize` (sem `cache:clear`) e o código não faz flush — **quem apagava a base 1 eram os deploys do Suporte**; perder a cache a meio de uma sincronização não estraga dados (fila e locks estão na base 0), perde-se o resumo da cadeia, o «em curso» e o «último sync» do painel, e o seguinte fica normal. De acordo com as duas propostas.
- Feito: `REDIS_CACHE_DB=2` no `.env` de produção (a base 2 estava vazia; verificado antes). A cache do Suporte (limites de pedidos e exportações, etc.) passou para lá; a Nexus Infra fica com a 1. As **sessões não mudam**: com o `SESSION_CONNECTION` vazio, o `SessionManager::createRedisDriver()` põe a ligação do store a `null` — a `default`, base 0 —, seja qual for a do store `sessao` (confirmado no código do Laravel e pela sessão da Nexus Infra). Não se mexeu em `SESSION_CONNECTION`, `REDIS_PREFIX` nem no prefixo do store `sessao` (pedido da Nexus Infra). A confirmar pelo utilizador: continua com sessão ao passar do portal para o Suporte.
- No repositório: `REDIS_CACHE_DB=2` no `.env.example`, e o `instalar.sh` recusa continuar sem ele.
- Aviso da Nexus Infra, que não é nosso: o **portal** também está na base 1 com `CACHE_STORE=redis` — convinha ir para uma base própria (por exemplo a 3) e nunca correr `cache:clear` lá. Regra da suite, como nas filas (§38): cada app com a sua base de cache.
